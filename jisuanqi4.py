...
def buju(root):
    ...
    label = Label(root,width=10, height=3, bd=0.5, bg='#EEE9E9', anchor='se',font=('微软雅黑', 20),textvariable=label_text)  # 标签，可以显示文字或图片
    label.grid(row=0, columnspan=10)  # 布局器，向窗口注册并显示控件； rowspan：设置单元格纵向跨越的列数

    entry = Entry(root, width=7, bd=15, bg='#EEE9E9', justify="right", font=('微软雅黑', 20))  # 文本框（单行）
    entry.grid(row=1, column=0, columnspan=10, sticky=N + W + S + E, padx=10, pady=10)  # 设置控件周围x、y方向空白区域保留大小

    myButton = partial(Button, root, width=5,height = 1,cursor='hand2', activebackground='#90EE90',fg = ('black'),font=('微软雅黑', 13))  # 偏函数：带有固定参数的函数
    button_sin = myButton(text='sin', command=lambda: get_input(entry, 'sin('),fg = ('#4F4F4F'))  # 按钮
    button_arcsin = myButton(text='arcsin', command=lambda: get_input(entry, 'arcsin('),fg = ('#4F4F4F'))
    button_exp = myButton(text='e', command=lambda: get_input(entry, 'e'),fg = ('#4F4F4F'))
    button_ln = myButton(text='ln', command=lambda: get_input(entry, 'ln('),fg = ('#4F4F4F'))
    button_xy = myButton(text='x^y', command=lambda: get_input(entry, '^'),fg = ('#4F4F4F'))
    ...
    button_shanquanbu = myButton(text=' AC ',command=lambda: clear(entry),fg = 'orange')# command指定按钮消息的回调函数
    ...
    button_shanquanbu.grid(row=3, column=0)
    button_shanyige.grid(row=3, column=1)
    ...
    button_dengyu = myButton(text=' \n = \n ', command=lambda: calculator(entry),bg = 'orange',font = ('黑体',16),height = 3)
...
def chakan(entry, argu):
    ...
    if argu == 'help':
        root.title('帮助')
        text.insert(INSERT, '暂时还没想到！\n')
        text.insert(INSERT, '敬请期待！')
    elif argu == 'author':
        root.title('作者')
        text.insert(INSERT, 'Author：Power\n')
        text.insert(INSERT, 'Time：2019-11-01')
...
if __name__ == '__main__':
    ...
    root.resizable(200,300)  # 窗口大小可调性，分别表示x，y方向的可变性
